...
export default class SessionController {
    ...
    try {
      try {
        await transport.sendMail({
          ...
          to: req.user.email,
          subject: "Se ha creado una cuenta en Limonero - Decoración para niños",
          text: `Bienvenidos a Limonero - Decoración para niños`,
        });
      } catch {}
  ...
  postRecoverPassword = async (req, res, next) => {
    try {
      ...
      if (result.acknowledged)
        res.send({ status: "Ok", message: "Contraseña modificada" });
    } catch (error) {
      next(error);
    }
  };
  ...
  postSwapUserClass = async (req, res, next) => {
    try {
      ...
      req.logger.debug("Datos del usuario");
